...
class IncrementalParsingTest extends AnyFunSuite {
  ...
  test("inserting between two empty braces works") {
    ...
    parser.changeRange(1, 1, 13, input2)
  ...
  test("removing works") {
    ...
    val parser = getParser
    val result = parser.parse(input)
    parser.changeRange(1,14, 0, input2)
    val result3 = parser.parse(input2)
  ...
  test("partial clearing works") {
    ...
    val parser = getParser
    parser.parse(input)
    parser.changeRange(1,4, 3, input2)
  ...
  test("inserts work") {
    ...
    val parser = getParser
    parser.parse(input)
    parser.changeRange(2,2,2, input2)
    val result = parser.parse(input2)
  ...
  test("multiple inserts work") {
    ...
    parser.parse(input)
    parser.changeRange(2,2,2, input2)
    parser.changeRange(6,6,2, input3)
