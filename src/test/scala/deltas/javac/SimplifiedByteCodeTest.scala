...
class SimplifiedByteCodeTest extends FunSuite {
    ...
    val utils = new TestUtils(CompilerBuilder.build(PresetsPanel.getJavaToSimplifiedByteCodePreset.deltas))
  ...
  test("simplifiedToByteCode") {
    val utils = new TestUtils(CompilerBuilder.build(PresetsPanel.getSimplifiedByteCodePreset.deltas))
