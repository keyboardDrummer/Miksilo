...
class TestByteCodeGoTo {
  ...
  def testPrintByteCode() = {
    ...
    val expectedHexWithoutWhiteSpace = expectedHex.split(Array(' ','\n')).mkString("")
    Assert.assertEquals(expectedHexWithoutWhiteSpace,valueOf(PrintByteCode.getBytes(getIndexedJumpWhile)))
  }
