...
object WhileContinueDelta extends DeltaWithPhase with DeltaWithGrammar {
  ...
  def transformProgram(program: Node, compilation: Compilation): Unit = {
    ...
    NodePathRoot(program).visitShape(ContinueKey, path => transformContinue(path, startLabels, compilation))
  }
  ...
  def addStartLabel(whilePath: NodePath): String = {
    ...
    whilePath.asInstanceOf[NodeSequenceElement].replaceWith(Seq(JustJavaLabel.label(startLabel), whilePath.current))
    startLabel
  }
