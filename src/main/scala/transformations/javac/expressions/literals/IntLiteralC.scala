...
import transformations.bytecode.coreInstructions.integers.{LoadConstantIntDelta$, SmallIntegerConstantDelta$}
...
object IntLiteralC extends ExpressionInstance {
  ...
  override def toByteCode(literal: Path, state: CompilationState): Seq[Node] = {
    ...
    {
      Seq(LoadConstantIntDelta$.integerConstant(IntegerInfoConstant.construct(value)))
    }
  }
