...
trait ConstantEntry extends DeltaWithGrammar {
  ...
  val key = Clazz //TODO kan vervangen worden
  def getByteCode(constant: Node, state: Language): Seq[Byte]

  override def inject(state: Language): Unit = {
    super.inject(state)
    ByteCodeSkeleton.getState(state).getBytes.put(Clazz, (constant: Node) => getByteCode(constant, state))
  }

  override def transformGrammars(grammars: GrammarCatalogue, state: Language): Unit = {
    val itemContent = grammars.find(ConstantPoolItemContentGrammar)
    itemContent.addOption(grammars.create(Clazz, getName ~~> getConstantEntryGrammar(grammars)))
  }
