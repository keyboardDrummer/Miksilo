...
object ElementaryTypeDelta extends DeltaWithGrammar with TypeInstance {
  ...
  override def getJavaGrammar(grammars: LanguageGrammars) = {
    ...
    val Uint = Seq("uint", "uint8", "uint16", "uint24", "uint32", "uint40", "uint48", "uint56", "uint64", "uint72", "uint80", "uint88", "uint96", "uint104", "uint112", "uint120", "uint128", "uint136", "uint144", "uint152", "uint160", "uint168", "uint176", "uint184", "uint192", "uint200", "uint208", "uint216", "uint224", "uint232", "uint240", "uint248", "uint256")
    ...
    val elementaryTypeNames = Seq("address", "string", "var") ++ Int ++ Uint ++ Seq("byte") ++ Byte
  ...
  override def getType(compilation: Compilation, builder: ConstraintBuilder, path: NodeLike, parentScope: Scope) = {
    PrimitiveType(path.getValue(Name).asInstanceOf[String])
  }
}
