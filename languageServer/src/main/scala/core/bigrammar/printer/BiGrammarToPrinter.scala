    ...
    printerCache.getOrElseUpdate(grammar, {
      val result: NodePrinter = grammar match {
        ...
        case leftRight: LeftRight =>
          ...
          val deconstruct = (withMap: AnyWithMap) => leftRight.bijective.destruct(withMap.value).map(v => WithMap(v, withMap.namedValues))
        ...
        case topBottom: TopBottom =>
          ...
          val deconstruct = (withMap: AnyWithMap) => topBottom.bijective.destruct(withMap.value).map(v => WithMap(v, withMap.namedValues))
          new MapGrammarWithMapPrinter(inner, deconstruct)
