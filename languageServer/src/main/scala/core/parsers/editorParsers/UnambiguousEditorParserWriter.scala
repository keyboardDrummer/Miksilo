...
trait UnambiguousEditorParserWriter extends UnambiguousParserWriter with EditorParserWriter {
  ...
  override def failure[Result](partial: Result, input: Input, message: String) = ParseFailure(Some(partial), input, message)

  override def success[Result](result: Result, remainder: Input) = ParseSuccess(result, remainder, NoFailure)
  ...
  trait EditorParseResult[+Result] extends UnambiguousParseResult[Result] {
