...
class GotoDefinitionTest extends FunSuite with LanguageServerTest {
  ...
  test("int variable") {
    ...
    assertResult(Seq(FileRange(itemUri, SourceRange(new HumanPosition(1,5), new HumanPosition(1,6)))))(result)
  }
  ...
  test("defined inside if") {
    ...
    assertResult(Seq(FileRange(itemUri, SourceRange(new HumanPosition(1,5), new HumanPosition(1,6)))))(xDefinition)

    val yDefinition = gotoDefinition(blockLanguage.language, program, new HumanPosition(4, 8))
    assertResult(Seq(FileRange(itemUri, SourceRange(new HumanPosition(3,7), new HumanPosition(3,8)))))(yDefinition)
  }
  ...
  test("fibonacci") {
    ...
    val methodRange = Seq(FileRange(itemUri, SourceRange(new HumanPosition(8, 23), new HumanPosition(8, 32))))
