...
class GotoDefinitionTest extends FunSuite with LanguageServerTest {
  ...
  test("fibonacci") {
    ...
    val indexDefinition = gotoDefinition(JavaToByteCodeLanguage.getJavaFrontend, program, new HumanPosition(10, 16))
    assertResult(Seq(Location(itemUri, Range(new HumanPosition(8,37), new HumanPosition(8,42)))))(indexDefinition)

    val fibonacciDefinition = gotoDefinition(JavaToByteCodeLanguage.getJavaFrontend, program, new HumanPosition(5, 36))
  ...
  test("assignment") {
    ...
    val myFieldDefinition = gotoDefinition(JavaToByteCodeLanguage.getJavaFrontend, program, new HumanPosition(11, 9))
