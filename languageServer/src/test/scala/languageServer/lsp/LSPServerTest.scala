...
import org.eclipse.lsp4j.{DiagnosticSeverity, MessageParams, ShowMessageRequestParams}
...
class LSPServerTest extends AsyncFunSpec {
  ...
  case class ServerAndClient(serverProxy: lsp4j.services.LanguageServer,
  ...
  it("can initialize") {
    ...
    val params = new lsp4j.InitializeParams()
    params.setCapabilities(new lsp4j.ClientCapabilities())
    params.setRootUri("someRootUri")
    val initializePromise = serverAndClient.serverProxy.initialize(params)

    val result = initializePromise.get()

    assert(result.getCapabilities == serverAndClient.server.in.getCapabilities(ClientCapabilities()))
...
//    }
//
//    val client = serverAndClient.serverProxy
...
//        |{"jsonrpc":"2.0","method":"textDocument/didOpen","params":{"textDocument":{"uri":"a","languageId":"","version":0,"text":"content"}}}""".stripMargin
//
//    val diagnostics = Seq(Diagnostic(new SourceRange(HumanPosition(0,1), HumanPosition(0, 5)),"Woeps", Some(DiagnosticSeverity.Warning)))
//    val p = Promise[Assertion]()
//    val languageClient = new TestLanguageClient {
//      override def sendDiagnostics(receivedDiagnostics: PublishDiagnosticsParams): Unit = {
...
//
//    val client = serverAndClient.serverProxy
//    client.getTextDocumentService.didChange(new lsp4j.DidChangeTextDocumentParams(
//      new lsp4j.VersionedTextDocumentIdentifier(document.uri, document.version), Seq.empty))
//
//    p.future
//  }
  ...
  private def setupServerAndClient(languageServer: LanguageServer,
                                   languageClient: LanguageClient = new TestLanguageClient()): ServerAndClient = {
    ...
    val server = new LanguageServerToLSP4JServer(languageServer)
    val connection = LSPLauncher.createServerLauncher(server, clientToServer.in, serverOut)
    val languageClientProxy = connection.getRemoteProxy
    server.setClient(languageClientProxy)
    connection.startListening()

    val clientLauncher = LSPLauncher.createClientLauncher(new LanguageClientToLSP4Client(languageClient), serverToClient.in, clientOut)
    clientLauncher.startListening()
    ServerAndClient(clientLauncher.getRemoteProxy, languageServer, clientOutCopy, serverOutCopy)
  }
  ...
  class TestLanguageServer extends LanguageServer {
    ...
    override def didChange(parameters: DidChangeTextDocumentParams): Unit = {
      client.sendDiagnostics(PublishDiagnosticsParams(parameters.textDocument.uri, getDiagnostics))
    }
