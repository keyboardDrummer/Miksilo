...
class YamlTest extends FunSuite
  ...
  lazy val plainStyleMultiLineString = new Sequence(new Sequence(nsPlainSafe, whiteSpace, (l: String, _: String) => l),
    greaterThan(WithIndentation(equal(nsPlainSafe).someSeparated("\n", "line"))),
  ...
  test("plainStyleMultineLineInFlowCollection") {
    val input =
      """/cloudformation_graphic.png" alt="AWS CloudFormation
        |         Logo"/""".stripMargin
    val result = plainStyleMultiLineString.getWholeInputParser()(new IndentationReader(input).withContext(FlowIn))
  ...
  test("plainStyleMultineLineInFlowCollection2") {
    ...
    val result = parseValue.getWholeInputParser()(new IndentationReader(input))
    assert(result.successful)
  }
  ...
  test("string") {
    ...
    val result = parseStringLiteral.getWholeInputParser()(new IndentationReader(program))
  ...
  test("number") {
    ...
    val result = parseNumber.getWholeInputParser()(new IndentationReader(program))
  ...
  test("object with single member") {
    ...
    val result = parseValue.getWholeInputParser()(new IndentationReader(program))
  ...
  test("big yaml file") {
    ...
    val result = parseValue.getWholeInputParser()(new IndentationReader(contents))
